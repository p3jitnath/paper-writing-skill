...
- Avoid clarification sentences built around `This is ... and not ...` or close variants. State the intended meaning directly. When a contrast is scientifically necessary, name the alternatives precisely and integrate the distinction naturally into the argument.
...
- Abstract length: mechanically verify after every abstract edit that its rendered prose contains no more than 250 words. Exclude LaTeX commands and the abstract environment delimiters from the count, include words displayed through command arguments, and apply any lower venue limit as well.
